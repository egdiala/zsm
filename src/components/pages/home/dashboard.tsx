  ...
  return (
    <div className={cn("flex flex-col p-6 gap-[3.125rem] rounded-2xl bg-white", className)}>
      <div className="flex items-start justify-between">
        ...
        <Button theme="arrow-cta-1" onClick={() => navigate("/dashboard")}>
          <span className="hidden lg:flex">View Dashboard</span>
        </Button>
      </div>
      <LineChart
        data={data}
            ...
            <div className="relative bg-grey-dark-1 p-2 flex flex-col gap-1.5 rounded text-white">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs w-16 text-right">
                  {(point.data.y as number).toFixed()} - {point.data.x as string}
                </span>
              </div>
